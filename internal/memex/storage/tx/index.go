...
// Index manages transaction index entries
type Index struct {
	entries []IndexEntry
	mutex   sync.RWMutex
}

// NewIndex creates a new transaction index
func NewIndex() *Index {
	return &Index{
...
// FindByStatus finds all entries with a given status
func (idx *Index) FindByStatus(status uint32) []IndexEntry {
	idx.mutex.RLock()
	defer idx.mutex.RUnlock()

	var matches []IndexEntry
	for _, entry := range idx.entries {
		if entry.Flags&0x0F == status { // Status is stored in lower 4 bits of flags
...
func (idx *Index) UpdateStatus(id [32]byte, status uint32) bool {
	idx.mutex.Lock()
	defer idx.mutex.Unlock()

	for i, entry := range idx.entries {
		if entry.ID == id {
			// Clear status bits and set new status
			entry.Flags = (entry.Flags &^ 0x0F) | (status & 0x0F)
			idx.entries[i] = entry
			return true
		}
	}
...
// PendingEntries returns all pending transaction entries
func (idx *Index) PendingEntries() []IndexEntry {
	return idx.FindByStatus(TxStatusPending)
}

// FailedEntries returns all failed transaction entries
func (idx *Index) FailedEntries() []IndexEntry {
	return idx.FindByStatus(TxStatusFailed)
}
